...
def parse(text):
    doc = pq(text)
    # 锁定页面中的img标签
    images = doc('div.slist ul li img').items()
    x = 0
    for image in images:
        # 获取每一张图片的链接
        url = "http://pic.netbian.com/4kdongman/"
        img_url = url+image.attr('src')
        # 获得每张图片的二进制内容
        img = requests.get(url=img_url, headers=headers, proxies=proxies).content
        # 定义要存储图片的路劲
        print(img)
        path = "E:\\img\\" + str(x) + ".jpg"
        # 将图片写入指定的目录 写入文件用"wb"
